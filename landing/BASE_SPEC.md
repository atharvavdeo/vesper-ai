# Base landing spec (verbatim from user — the hero viewport must match this exactly)

## USER OVERRIDES (take precedence over the base spec below)
1. Hero background = this video, 100% opacity, **no overlay / no scrim** (the `.hero-photo::after` scrim is dropped):
   ...
   (autoplay muted loop playsinline, object-fit cover, fixed/absolute behind hero). This supersedes "Do not add a video".
2. Modify all copy/details to fit OUR product (voice-led operational memory for construction site managers — see ../README.md problem statement).
3. EXTEND the page below the hero viewport with other relevant sections, using animations, carousels and parallax in this exact theme and tone.
   This supersedes "single-viewport / no extra sections / desktop no-scroll lock". The hero itself must still occupy exactly one viewport and look identical in structure.
...
**1. Hero video background 100% opacity no overlay**
...
1. `html/body` black
2. `.hero-photo` + `::after` scrim   (OVERRIDE: video, no scrim)
...
**≥901 desktop lock:** `html, body { height: 100%; overflow: hidden }`. `.page { height: 100vh / 100dvh; overflow: hidden }`. One frame, **no scroll**.  (OVERRIDE: remove the scroll lock; hero viewport stays exactly 100vh/100dvh, page scrolls into extension sections.)
...
(OVERRIDE: additional small JS allowed for scroll-reveal, parallax, carousels — vanilla, no libraries, rAF-throttled, reduced-motion respected.)
