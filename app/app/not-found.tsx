/* Custom 404 for the Next routes; the static Cloudflare build serves public/404.html.
   Generated from that file — styles are scoped to .vs404 so they cannot leak into the shell. */
export default function NotFound() {
  ...
  * { box-sizing: border-box; }
  ...
  .vs404 .aura { position: fixed; inset: 0; pointer-events: none; z-index: 0; }
  .vs404 .aura::before, .aura::after {
  ...
  /* The stack must stay a grid: every layer sits in the same cell so the four materials
     overlap into one glyph. Do not give .glass > span a display of its own — it outranks
     .stack on specificity and the layers fall into a vertical column. */
  .vs404 .glass .stack { display: grid; place-items: center; position: relative; }
  .vs404 .glass .layer { grid-area: 1 / 1; position: relative; }
  ...
  .vs404 .copy { max-width: 46ch; display: grid; gap: 12px; opacity: 0; animation: rise 0.9s var(--ease) 0.34s forwards; }
  .vs404 .copy h1 { margin: 0; font-size: clamp(19px, 2.9vw, 27px); font-weight: 600; letter-spacing: -0.02em; }
  .vs404 .copy p { margin: 0; color: var(--muted); font-size: clamp(14px, 1.8vw, 16px); line-height: 1.6; }
  .vs404 .stamp {
  ...
  .vs404 .actions { display: flex; flex-wrap: wrap; gap: 12px; justify-content: center;
  ...
  .vs404 .btn:hover, .btn:focus-visible { transform: translateY(-2px); }
  .btn-solid { background: #ffffff; color: #000000; border-color: #ffffff; }
  .btn-solid:hover, .btn-solid:focus-visible { background: rgba(255,255,255,0.88); }
  .btn-ghost { color: var(--text); background: rgba(255,255,255,0.04); }
  .btn-ghost:hover, .btn-ghost:focus-visible { background: rgba(255,255,255,0.09); border-color: rgba(255,255,255,0.3); }
  ...
  @media (prefers-reduced-motion: no-preference) {
    .vs404 .glass { animation: rise 1.1s var(--ease) 0.14s forwards, bob 7s var(--ease) 1.2s infinite; }
  }
  @media (prefers-reduced-motion: reduce) {
    .sheen { animation: none; background-position: -150% 0; }
    .vs404 .eyebrow, .glass, .copy, .actions { opacity: 1; animation: none; }
  }
` }} />
  ...
  <main>
    ...
    <div className="glass" role="img" aria-label="404">
      <span className="stack" aria-hidden="true">
        <span className="layer caustic">404</span>
        <span className="layer depth">404</span>
        <span className="layer face">404</span>
        <span className="layer sheen">404</span>
      </span>
    </div>
    ...
    <div className="copy">
      ...
      <p className="stamp">Status: Superseded · Latest revision: the home page</p>
    </div>

    <div className="actions">
      <a className="btn btn-solid" href="/">Back to safety</a>
      <a className="btn btn-ghost" href="/app">Launch App</a>
    </div>
  </main>
    </div>
  );
